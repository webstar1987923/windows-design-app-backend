...
class Notes
{
    ...
    public function add(array $data)
    {
        ...
        if (!isset($data['content'])) {
            throw new \Exception('You must include a "content" field when inserting a note');
        }
        ...
        if (!isset($data['deal_id']) && !isset($data['person_id']) && !isset($data['org_id'])) {
            throw new \Exception('You must include one of the following "deal_id", "person_id", "org_id" field when inserting a note');
        }
