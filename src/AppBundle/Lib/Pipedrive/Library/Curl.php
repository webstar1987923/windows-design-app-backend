...
class Curl
{
     ...
     * @return array  decoded Json Output
     */
    public function post_files($method, array $data, array $files)
    {
        //set cURL transfer option for post request
        // and get ouput
        return $this->createEndPoint($method)
                    ->curl_custom_postfields($data, $files)
                    ->exec();
    }
    ...
    protected function exec()
    {
        ...
        if (curl_error($this->curl)) {
            //throw error
            throw new \Exception('API call failed: ' . curl_error($this->curl));
        }
        //decode output
        ...
        if (floor($info['http_code'] / 100) >= 4) {
            //throw error
            throw new \Exception('API HTTP Error ' . $info['http_code'] . '. Message ' . $result['error']);
        }
        // return output
        return $result;
    }
            ...
            if ($this->isArrayMultiDim($data)) {
                ...
                foreach ($data as $key => $value) {
                    ...
                    elseif (is_string($value) && strpos($value, '@') === 0) {
                        // add file
                        // TODO: Uncomment this if PHP >= 5.5.0
